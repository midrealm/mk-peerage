  ...
  describe "put /chambers/#{peerage}/admin/peers/:id" do
    context "for signed in #{peerage} admin" do
      ...
      it "update profile for signed in #{peerage} admin, and redirects to profile page" do
        ...
        put "/chambers/#{peerage}/admin/peers/#{newly_deceased.peers.first.id}", params: { peerage => {deceased: true} }
        expect(response).to redirect_to "/chambers/#{peerage}/admin/peers"
    ...
    it "redirects if not logged in" do
      newly_deceased = create(peerage, sca_name: "Newly Deceased", deceased: false)
      put "/chambers/#{peerage}/admin/peers/#{newly_deceased.id}", params: { peerage => {deceased: true} }
