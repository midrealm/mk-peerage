shared_examples_for "post /chambers/PEERAGE/admin/PEERS" do |peerage,other_peerage|
  describe "post /chambers/#{peerage}/admin/#{peerage}s" do
    context "for signed in admin #{peerage}" do
      ...
      it "creates new #{peerage}" do
        ...
        post "/chambers/#{peerage}/admin/#{peerage}s", params: @params
      ...
      it "handles true for vigilant for new #{peerage}" do
        post "/chambers/#{peerage}/admin/#{peerage}s", params: { peerage.to_sym => {id: "", sca_name: "Dingus McDOOOGLE", vigilant: true, email: "dingus@doogleson.com"} }
        expect(Peer.last.vigilant).to be_truthy 
        
      end
      it "handles false for vigilant for new #{peerage}" do
        post "/chambers/#{peerage}/admin/#{peerage}s", params: { peerage.to_sym => {id: "", sca_name: "Dingus McDOOOGLE", vigilant: false, email: "dingus@doogleson.com"} }
        expect(Peer.last.sca_name).to eq("Dingus McDOOOGLE") 
        expect(Peer.last.vigilant).to be_falsey 
      end
      it "shows generates slug for new #{peerage}" do
        post "/chambers/#{peerage}/admin/#{peerage}s", params: { peerage.to_sym => {id: "", sca_name: "Heregyð Ketilsdóttir", email: "dingus@doogleson.com"} }
        expect(User.last.slug).to eq("heregyd_ketilsdottir")
      end
      it "sends welcome and change password email to new #{peerage}" do
        post "/chambers/#{peerage}/admin/#{peerage}s", params: { peerage.to_sym => {id: "", sca_name: "Dingus McDOOOGLE", email: "dingus@doogleson.com"} }
    ...
    it "doesn't allow non admins to create new users" do
      ...
      sign_in(non_admin)    
      expect{post "/chambers/#{peerage}/admin/#{peerage}s", params: { peerage.to_sym => {sca_name: "Dingus McDOOOGLE", email: "dingus@doogleson.com"} } }.to raise_error(CanCan::AccessDenied)
    end
  end
end
