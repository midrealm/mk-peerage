...
describe 'Get /pelican/groups' do
  ...
  it 'does not show non-pelicans' do
    group = create(:group, name: 'High Haven', slug: 'high_haven')
  ...
  it 'shows list of child groups' do
    ...
    get '/pelican/groups/high_haven'
    expect(response.body).not_to include('/pelican/groups/high_haven')
    expect(response.body).to include('/pelican/groups/the_barrows')
    expect(response.body).to include('/pelican/groups/poopland')
  end
end
