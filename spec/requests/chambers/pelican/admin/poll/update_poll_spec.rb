...
describe "put /chambers/pelican/admin/poll" do
  it "updates poll, and redirects to dashboard" do
    ...
    put "/chambers/pelican/admin/poll", params: { :poll => {start_date: start_date, end_date: end_date} }
  ...
  it "updates poll for current poll" do
    ...
    end_date = (DateTime.now + 4.days).strftime('%d-%b-%Y')
    put "/chambers/pelican/admin/poll", params: { :poll => { end_date: end_date} }
    expect(response).to redirect_to "/chambers"
    expect(Poll.last.end_date.strftime('%d-%b-%Y')).to eq(end_date)
  end

  it "redirects if not logged in" do
    poll = create(:poll, peerage_type: :pelican)
    put "/chambers/pelican/admin/poll", params: { :poll => {start_date: '', end_date:''} }
