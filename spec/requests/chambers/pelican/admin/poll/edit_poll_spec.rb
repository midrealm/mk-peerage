...
describe "Get /chambers/pelican/admin/poll/edit" do
  describe "for logged in admin" do 
    before(:each) do
      admin = create(:pelican_admin)
      sign_in(admin)
      @future_poll = build(:future_poll, peerage_type: :pelican)
      @current_poll = build(:current_poll, peerage_type: :pelican)
      @past_poll = build(:past_poll, peerage_type: :pelican)
    end
    it "shows edit page for future poll" do
      @future_poll.save
      get "/chambers/pelican/admin/poll/edit"
      expect(response).to have_http_status(:success)
      expect(response.body).to include('Edit Pelican Poll')
    end
