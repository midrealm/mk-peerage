...
describe "Get /chambers/pelican/candidates/:id/poll_comments" do
  before(:each) do
  ...
  context 'logged in royal' do
    ...
    context 'past poll result' do
      before(:each) do
        ...
        @advising = create(:advising, poll: @p, peer: @pelican.pelican, candidate: @candidate,
      ...
      it "shows poll comment page for candidate with poll result" do
        get "/chambers/pelican/candidates/#{@candidate.id}/poll_comments"
    ...
    context 'no past poll result' do
      it "redirects to candidates index" do
        get "/chambers/pelican/candidates/#{@candidate.id}/poll_comments"
        expect(response).to redirect_to "/chambers/pelican/candidates"
      end
    end
  end
  it "raises AccessDenied Error for non-royal pelican" do
    @pelican = create(:pelican)
    sign_in(@pelican)
    expect{get "/chambers/pelican/candidates/#{@candidate.id}/poll_comments"}.to raise_error(CanCan::AccessDenied)
  end
  it "raises AccessDenied Error for non-royal admin pelican" do
    @non_royal_admin = create(:pelican_admin)
